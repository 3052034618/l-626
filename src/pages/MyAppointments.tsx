                  ...
                  <div className="flex items-center gap-5">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2.5 mb-1 flex-wrap">
                        ...
                        <span className={`px-2 py-0.5 rounded-full text-[11px] font-bold border flex-shrink-0 ${
...
function AppointmentDetail({
  ...
  const qrExpired = useMemo(
    () => appt.status === 'approved' && new Date(appt.expiresAt).getTime() < Date.now(),
    [appt]
  );
      ...
      <div className="w-full max-w-2xl bg-white rounded-3xl shadow-card-hover p-8 animate-fade-up">
        ...
        <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {(appt.qrCodeHistory?.length || 0) > 1 && (
              <span className="px-3 py-1 rounded-full text-xs font-bold bg-primary-100 text-primary-700 border border-primary-200 flex items-center gap-1">
                <Sparkles className="w-3 h-3" />新二维码
              </span>
            )}
          </div>
        </div>
        ...
        {showQr && (
          <div className="flex justify-center mb-6">
            <div className={`p-5 rounded-2xl border-2 ${
              qrExpired ? 'border-ink-200 bg-ink-50' :
              appt.status === 'approved' ? 'shadow-glow border-accent/30' :
              'border-ink-200 opacity-75'
            } relative`}>
              {qrExpired && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 rounded-full bg-ink-200 text-ink-700 text-[11px] font-bold flex items-center gap-1">
                  <Clock4 className="w-3 h-3" />二维码已过期
                </div>
              )}
              {(appt.qrCodeHistory?.length || 0) > 1 && !qrExpired && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 rounded-full bg-gradient-to-r from-primary-500 to-accent text-white text-[11px] font-bold flex items-center gap-1 shadow-md">
                  <Sparkles className="w-3 h-3" />新二维码
                </div>
              )}
              ...
              {qrExpired && (
                <div className="text-center text-xs text-ink-400 mt-1">请改期后获取新二维码</div>
              )}
            </div>
          </div>
        )}
        ...
        {(appt.qrCodeHistory?.length || 0) > 1 && (
          <div className="mt-6">
            <h4 className="text-sm font-bold text-ink-700 mb-3 flex items-center gap-1.5">
              <QrCode className="w-4 h-4 text-primary-600" />二维码历史
            </h4>
            <div className="space-y-2">
              {appt.qrCodeHistory?.map((item, i) => (
                <div key={i} className={`text-xs rounded-lg px-3 py-2 flex items-center gap-3 ${
                  item.qrCode === appt.qrCode ? 'bg-primary-50 border border-primary-200' : 'bg-ink-50'
                }`}>
                  <div className={`w-10 h-10 rounded-md bg-white border border-ink-200 flex items-center justify-center flex-shrink-0 ${
                    item.qrCode === appt.qrCode ? '' : 'opacity-50'
                  }`}>
                    <QRCodeSVG value={item.qrCode} size={32} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      <span className="font-bold text-ink-700 font-mono text-[11px] truncate">
                        {item.qrCode}
                      </span>
                      {item.qrCode === appt.qrCode && (
                        <span className="px-1.5 py-0.5 rounded bg-primary-500 text-white text-[10px] font-bold flex items-center gap-0.5">
                          <Sparkles className="w-2.5 h-2.5" />当前使用
                        </span>
                      )}
                    </div>
                    <div className="text-ink-400 flex items-center gap-2">
                      <span>{item.reason === 'create' ? '创建生成' : item.reason === 'reschedule' ? '改期生成' : '补发生成'}</span>
                      <span>·</span>
